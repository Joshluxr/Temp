/**
 * STIX 2.1 + MISP event export for intel findings.
 *
 * Findings are converted into both:
 *   - STIX 2.1 bundle (https://oasis-open.github.io/cti-documentation/stix/intro)
 *   - MISP event JSON (https://www.misp-project.org/)
 *
 * The two share a single internal model so they always represent the same
 * finding set. We map a finding to:
 ...
 *
 * Output is deterministic given a fixed `now` and `bundleId` so tests can
 * snapshot the JSON. The `uuid()` helper is injectable for determinism.
 */
...
export interface StixExportInput {
  readonly findings: readonly IntelFinding[];
...
/** Injectable UUID generator (default: crypto-strength random). */
...
export function detectCves(text: string): string[] {
  const re = /CVE-\d{4}-\d{4,7}/gi;
  const matches = text.match(re);
  return matches ? Array.from(new Set(matches.map(m => m.toUpperCase()))) : [];
}

export function detectIocs(text: string): { kind: 'url' | 'domain' | 'ipv4'; value: string }[] {
  ...
  for (const d of domains) {
    // Skip values already captured inside URLs / IPs.
...
export function exportStixBundle(input: StixExportInput, uuid: UuidFn = randomUuid): StixBundle {
  ...
  for (const f of input.findings) {
    ...
    objects.push({
      ...
      created: now,
      modified: now,
      name: cves.length > 0 ? `${f.title} (${cves[0]})` : f.title,
      description: `source=${f.source} target=${f.target} severity=${f.severity}${evidence ? ` evidence=${evidence}` : ''}`,
      labels: [f.severity, f.source],
      ...(cves.length > 0
...
function mispThreatLevel(severity: IntelSeverity): '1' | '2' | '3' | '4' {
...
export function exportMispEvent(input: StixExportInput): MispEvent {
  ...
  let worstSeverity: IntelSeverity = 'info';
  const sevRank: Record<IntelSeverity, number> = { info: 0, low: 1, medium: 2, high: 3, critical: 4 };
  for (const f of input.findings) {
